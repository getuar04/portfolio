          ...
          <div className="relative grid lg:grid-cols-2 gap-12 items-start">
            ...
            <div className="grid gap-3">
              {contactItems.map((item, i) => {
                const inner = (
                  <div
                    className="flex items-center gap-4 glass rounded-[20px] p-4 card-hover"
                    key={item.key}
                  >
                    <div
                    ...
                    <div>
                      ...
                      <p
                        ...
                        style={item.href ? { color: "var(--accent-light)" } : {}}
                      >
                        {item.value}
                      </p>
                    </div>
                    {item.href && (
                      <svg className="w-4 h-4 ml-auto" style={{ color: "rgba(255,255,255,0.2)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                return item.href ? (
                  <a
                    ...
                    className={`reveal ${visible ? "visible" : ""} reveal-delay-${i + 1} block`}
                  >
                    {inner}
                  </a>
                ) : (
                  <div key={item.key} className={`reveal ${visible ? "visible" : ""} reveal-delay-${i + 1}`}>
                    {inner}
                  </div>
                );
              })}
