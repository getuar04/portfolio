...
const FILTERS = ["all", "featured", "backend", "fullstack", "frontend", "university", "professional", "research"];
...
function ProjectLinks({ project, t, size = "text-xs px-4 py-2" }) {
  const hasAny = project.github || project.live || project.linkedin || project.screenshots;
  return (
    <div className="flex flex-wrap gap-2">
      {project.github && (
        <a href={project.github} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className={`btn-ghost ${size}`}>
          {githubIcon} {t.projects.viewGithub}
        </a>
      )}
      {project.live && (
        <a href={project.live} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className={`btn-primary ${size}`}>
          {externalIcon} {t.projects.viewLive}
        </a>
      )}
      {project.linkedin && (
        <a href={project.linkedin} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className={`btn-ghost ${size}`} style={{ color: "#38bdf8" }}>
          {linkedinIcon} {t.projects.presentation}
        </a>
      )}
      {project.screenshots && (
        <a href={project.screenshots} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className={`btn-ghost ${size}`}>
          {externalIcon} {t.projects.viewScreenshots}
        </a>
      )}
      {!hasAny && (
        <span className={`btn-ghost ${size} opacity-50 cursor-default select-none`}>{t.projects.private}</span>
      )}
    </div>
  );
}
...
function ProjectCard({ project, onOpen, lang, t, delay }) {
  ...
  return (
    <article className={`reveal visible reveal-delay-${delay} glass rounded-[24px] p-6 card-hover h-full flex flex-col`}>
      ...
      <p className="text-sm leading-7 mb-5 flex-1" style={{ color: "var(--ink-3)" }}>
        {description}
      </p>
  ...
  return (
    ...
    >
      <div
        ...
        tabIndex={-1}
        className="glass rounded-[28px] w-full max-w-2xl p-6 sm:p-8 relative overflow-y-auto"
        style={{ boxShadow: "0 0 0 1px rgba(124,58,237,0.2), 0 40px 100px rgba(0,0,0,0.5)", maxHeight: "88vh" }}
      ...
      >
        ...
        <div className="flex flex-col gap-5 mb-6">
          {overview && (
            <div>
              ...
              <p className="text-sm leading-7" style={{ color: "var(--ink-2)" }}>{overview}</p>
            </div>
          )}
        ...
        <ProjectLinks project={project} t={t} size="text-sm px-5 py-2.5" />
      </div>
    </div>
  );
}
